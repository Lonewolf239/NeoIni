...
#if NETSTANDARD2_0
        internal async Task<Releaser> ReadLockAsync(CancellationToken ct = default) => await AcquireAsync(false, ct);
#else
...
#if NETSTANDARD2_0
        internal async Task<Releaser> WriteLockAsync(CancellationToken ct = default) => await AcquireAsync(true, ct);
#else

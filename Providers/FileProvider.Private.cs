...
#if NETSTANDARD2_0
            byte[] dataWithoutChecksum = new byte[data.Length - ChecksumSize];
            Array.Copy(data, 0, dataWithoutChecksum, 0, dataWithoutChecksum.Length);
            byte[] expectedChecksum = new byte[ChecksumSize];
            Array.Copy(data, dataWithoutChecksum.Length, expectedChecksum, 0, ChecksumSize);
            byte[] calculatedChecksum;
            using (var sha256 = SHA256.Create())
                calculatedChecksum = sha256.ComputeHash(dataWithoutChecksum);
#else

...
| 3.3 | ✅ Released | **Пакетные операции**: методы типа `SetValuesAsync` для атомарного обновления нескольких ключей за одну операцию, снижая накладные расходы автосохранения. |
| 3.4 | 🔵 Under consideration | **Поддержка потоковых провайдеров**: возможность чтения/записи больших конфигураций инкрементально, без загрузки всего набора данных в память. |

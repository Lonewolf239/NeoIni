...
[![xUnit Тесты](https://img.shields.io/badge/xUnit-Тесты-2D2D2D?style=for-the-badge&logo=xunit&logoColor=FFFFFF)](https://lonewolf239.github.io/NeoIni/)
...
**Безопасный, потокобезопасный конфигурационный фреймворк для .NET с INI-подобным хранением** — включает AES-256 шифрование, SHA-256 контрольные суммы, горячую перезагрузку, подключаемые провайдеры и атомарное резервное копирование.

> ⚠️ **Не классический парсер INI** – NeoIni — это **система управления конфигурацией**, которая использует расширенный INI-формат в качестве слоя сериализации. Файлы, созданные в стандартном режиме, содержат бинарные заголовки, контрольные суммы и метаданные шифрования, что делает их непригодными для прямого ручного редактирования (используйте Human Mode для конфигов, редактируемых вручную).
...
NeoIni распространяется под лицензией **GPLv3**. Если эта лицензия не подходит для ваших целей (например, для проприетарного или коммерческого ПО), вы можете приобрести **коммерческую лицензию**.

Коммерческая лицензия позволяет использовать NeoIni в проприетарных приложениях без необходимости раскрывать исходный код. Включены приоритетная поддержка и доступ к приватным обновлениям.

**Цены:**

| Тип лицензии   | Цена     | Включает                                                                                           |
| -------------- | -------- | -------------------------------------------------------------------------------------------------- |
| Индивидуальная | **$49**  | Для одного разработчика, один продукт.                                                             |
| Командная      | **$199** | До 5 разработчиков, до 3 продуктов.                                                                |
| Корпоративная  | **$499** | Неограниченное количество разработчиков и продуктов, один год приоритетной поддержки и обновлений. |

Все лицензии бессрочные (единоразовый платёж). Годовая подписка на поддержку и обновления доступна за 20% от стоимости лицензии.

Для приобретения или индивидуальных условий лицензирования свяжитесь с разработчиком:

- **Telegram:** [@an1onime](https://t.me/an1onime)
- **Email:** neoinilicense@gmail.com

_Цены указаны в USD и могут быть изменены._
...
**Стандартный режим против Human Mode** – В стандартном режиме NeoIni полностью управляет конфигурационным файлом: добавляет бинарный заголовок, контрольные суммы и опциональное шифрование. Это гарантирует целостность, но делает файл непригодным для ручного редактирования (в файл вставляется предупреждающий заголовок). Для INI-файлов, редактируемых вручную, используйте **Human Mode** (экспериментальный режим), который отключает контрольные суммы и шифрование, сохраняя комментарии и форматирование.

Таким образом, NeoIni — это не замена легковесным парсерам INI, а **надёжная конфигурационная подсистема**, которая в качестве формата сериализации использует текстовый формат, напоминающий INI.

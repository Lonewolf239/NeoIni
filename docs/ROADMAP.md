...
| 3.2     | ✅ Released            | **.NET Standard 2.0 support**: enable usage on .NET Framework 4.6.2+ and other legacy platforms by backporting async/await and other modern features. |
| 3.3     | ✅ Released            | **Batch operations**: methods like `SetValuesAsync` to update multiple keys in a single atomic operation, reducing auto‑save overhead.                |
| 3.x     | 🔵 Under consideration | **Streaming provider support**: allow large configurations to be read/written incrementally without loading the entire dataset into memory.           |
| 3.x     | 🔵 Under consideration | **Memory-mapped I/O**: optionally use memory-mapped files for very large INI files to improve performance and reduce memory footprint.                |

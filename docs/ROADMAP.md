...
| Version | Status | Description |
|---------|--------|-------------|
| 3.1 | 🟡 Planned | **Streaming provider support**: allow large configurations to be read/written incrementally without loading the entire dataset into memory. |
| 3.2 | 🔵 Under consideration | **Extended source generator**: support for nested objects, collections, and validation attributes (e.g., `[Range]`, `[Required]`) in the generated mapping code. |
| 3.3 | 🔵 Under consideration | **Memory-mapped I/O**: optionally use memory-mapped files for very large INI files to improve performance and reduce memory footprint. |
| 3.4 | 🔵 Under consideration | **Batch operations**: methods like `SetValuesAsync` to update multiple keys in a single atomic operation, reducing auto‑save overhead. |
| 3.5 | 🔵 Under consideration | **.NET Standard 2.0 support**: enable usage on .NET Framework 4.6.2+ and other legacy platforms by backporting async/await and other modern features. |

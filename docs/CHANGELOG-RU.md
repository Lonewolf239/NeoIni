...
- **Переработано автоматическое («привязанное к машине») шифрование**
    - Ключ больше не выводится из имени пользователя, имени машины и домена — эта схема защищала только от случайного подглядывания, так как эти значения часто видны по сети или легко угадываются.
    - Теперь он выводится из случайного 256-битного секрета, сгенерированного один раз и хранящегося локально (на Windows защищён через DPAPI, scope `LocalMachine`), в сочетании со стабильным аппаратным/установочным идентификатором (`MachineGuid` из реестра на Windows, `/etc/machine-id` на Linux).
    - Всё так же полностью автоматическое — пароль никогда не запрашивается — и всё так же открывается только на той машине, где создавалось, но больше не восстанавливается из публично доступной информации.
- **Версия формата файла поднята до 2** — отражает новую схему авто-шифрования.
- **Добавлена прозрачная миграция**: при открытии файла версии 1 с авто-шифрованием он расшифровывается по старой схеме и, если включён `UseAutoSave`, сразу пересохраняется по новой (пропускается, если `UseAutoSave` выключен, например через `NeoIniOptions.ReadOnly`, — миграция никогда не выполняет запись, которую вызывающий код явно отключил). Файлы на основе пароля (`EncryptionType.Custom`) это изменение не затрагивает.
- **Исправлен краш** (`IOException: File too small to read the requested range`) при создании зашифрованного документа поверх уже существующего пустого или обрезанного файла — `GetSalt` теперь трактует это как «заголовка нет», а не пробрасывает исключение.
- Исправлена ссылка тестового проекта на устаревший опубликованный пакет `NeoIni` вместо локального кода — тесты фактически не проверяли локальные изменения.

</details>

<details>
<summary><strong>3.4.4</strong> — 8 августа 2026</summary>

#### List of changes

- **Исправлена гонка при сохранении файла**
    - Добавлен внутренний семафор, чтобы автосохранение, ручные `SaveFile`/`SaveFileAsync` и `Dispose`/`DisposeAsync` больше не писали одновременно в один и тот же временный файл.
    - Конкурентные запросы на автосохранение, поступившие пока уже идёт сохранение, больше не теряются молча — текущее сохранение теперь делает дополнительный проход, чтобы их подхватить.
- **Более безопасная обработка ошибок в `Dispose`/`DisposeAsync`**
    - Ошибки сохранения при освобождении ресурсов теперь всегда трассируются (ранее — только в `DEBUG`) и передаются через `Error`, если на него есть подписчик, вместо полного отсутствия сигнала в Release-сборках.
- **Единообразное поведение исключений**
    - `AddKey`, `RenameKey` и `RenameSection` (и их async-версии) теперь всегда выбрасывают `InvalidOperationException` при конфликте, независимо от наличия подписчика на `Error`.
    - Конструкторы, принимающие пользовательский `IEncryptionProvider`, теперь во всех случаях выбрасывают `ArgumentNullException` при `null`, вместо тихой подстановки провайдера по умолчанию в двух из шести перегрузок.
- **Исправлен пробел в валидации `CreateHumanMode`/`CreateHumanModeAsync`**
    - Передача `NeoIniOptions.UseShielding = true` через `options` больше не обходит проверку `ModeConflictException`, обязательную для Human Mode.
- **Исправлены перепутанные значения в `ChecksumMismatchEventArgs`**
    - `Expected`/`Actual` заполнялись в обратном порядке при несовпадении контрольной суммы.
- **Исправлено вводящее в заблуждение исключение в `DeleteFile`**
    - При вызове на не-файловом провайдере ранее сообщалось об ошибке, связанной с бэкапом; теперь сообщение описывает реальную причину.
- **Обновление зависимости**
    - `AsyncReaderWriterLock` обновлён с `1.0.2` до `1.0.3` — исправлена гонка, при которой блокировка могла быть выдана (или ожидающий навсегда зависал), если `Dispose()` срабатывал во время асинхронного захвата на медленном пути.
- **Документация**
    - Обновлены XML doc-комментарии по всему публичному API для точности и единообразия.
    - Синхронизированы гайды `docs/*.md` с исправленным поведением (количество событий, заметки об обработке ошибок, гайд по провайдерам, заметки про hot-reload, таблица поддержки в security).
...
- **Обновление документации**
    - Уточнено, что NeoIni — это конфигурационный фреймворк с INI-подобной сериализацией, а не классический парсер INI.
...
- **Internal класс вынесен в отдельную библиотеку** – класс `AsyncReaderWriterLock` (ранее `internal`) перемещён в отдельный NuGet-пакет `AsyncReaderWriterLock`.

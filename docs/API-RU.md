...
| `GetEncryptionPassword` | Получение автосгенерированного пароля шифрования (бросает исключение, если недоступен)         | –                         |
...
| `UseShielding`     | Позволяет использовать значения в кавычках (например, key = "value ; not a comment"); нельзя сочетать с Human Mode | `false`            |
...
> **Примечание:** Ошибки на уровне провайдера (файловый I/O, шифрование, парсинг) сообщаются через событие `Error`; если на него никто не подписан, вместо этого бросается само исключение. `AddKey`, `RenameKey` и `RenameSection` (и их async-версии) при конфликте **всегда** бросают `InvalidOperationException` — они дополнительно уведомляют `Error`, если на него есть подписчик, но подписка не подавляет исключение.

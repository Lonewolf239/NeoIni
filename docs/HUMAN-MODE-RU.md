...
- **Без checksum:** Свойство `UseChecksum` игнорируется, checksum никогда не вычисляется и не проверяется.
- **Без шифрования:** AES-256 шифрование недоступно в human mode.
- **Без экранирования:** `UseShielding` не может быть включён, так как экранирование кавычек несовместимо с ручным редактированием

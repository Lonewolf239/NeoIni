...
> **Примечание:** Методы `Encrypt`/`EncryptAsync` и `Decrypt`/`DecryptAsync` доступны начиная с версии **3.1**. Если вы используете более старую версию, обновитесь до 3.1, чтобы использовать полный интерфейс провайдера шифрования.
...
- **Авто‑шифрование:** ключ выводится из привязанного к машине секретного материала (непереносимый). См. ниже.
- **На основе пароля:** ключ выводится из строки, предоставленной пользователем, через PBKDF2 (320 000 итераций).

---

### Как авто-шифрование выводит ключ (с версии 3.5)

Раньше автоматический ключ выводился из имени пользователя, имени машины и домена — эти значения часто видны по сети или легко угадываются, поэтому файл, который лишь _выглядел_ привязанным к машине, на практике можно было расшифровать где угодно, если атакующий мог воспроизвести эту строку. С версии **3.5** ключ выводится из:

- случайного 256-битного секрета, сгенерированного один раз и хранящегося локально (на Windows защищён через **DPAPI**, scope `LocalMachine`), и
- стабильного аппаратного/установочного идентификатора (`MachineGuid` из реестра на Windows, `/etc/machine-id` на Linux).

Ни одно из этих значений не видно при осмотре самого зашифрованного файла и не выводится из публичной информации о машине — для воспроизведения ключа нужен локальный доступ именно к этой машине. Это осознанный компромисс: он не защищает (и не может защитить) от атакующего, у которого уже есть тот же уровень доступа, что и у процесса, создавшего файл, но он убирает гораздо более широкую поверхность атаки «угадать имя пользователя и хоста».

**Версия формата и миграция:** байт версии формата файла поднят до `2`. Файлы, зашифрованные старой (версии 1) автоматической схемой, по-прежнему читаются прозрачно — NeoIni определяет версию, расшифровывает по старой схеме и, если включён `UseAutoSave` (по умолчанию так и есть), сразу пересохраняет файл по новой схеме. Если `UseAutoSave` отключён (в том числе через `NeoIniOptions.ReadOnly`), сохранение-миграция пропускается, чтобы не нарушать эту настройку — файл всё равно корректно расшифровывается и используется, но остаётся версии 1, пока не будет открыт там, где сохранение возможно. Файлы на основе пароля (`EncryptionType.Custom`) это изменение не затрагивает, так как они никогда не зависели от идентичности машины.
...
- Пользовательские реализации `IEncryptionProvider` не затрагиваются повышением версии файла — переключение между старой и новой авто-схемой применяется только к встроенному `NeoIniEncryptionProvider`.

> **Примечание:** В версиях до 2.0 шифрование было жёстко привязано к AES‑256‑CBC. Перейдите на версию 3.1, чтобы воспользоваться новой расширяемой архитектурой.
